...
const CHECKLISTS = {
    ...
    sexual: ['Medical report collection', 'Witness statements', 'Scene examination', 'Victim statement (CrPC 164)', 'Forensic evidence collection'],
...
const LEAD_TEMPLATES = {
    theft: [
        ...
        { text: 'Verify alibi of persons with prior theft convictions in sector', weight: 0.6 }
    ],
    assault: [
    ...
    fraud: [
        { text: 'Freeze suspect bank accounts immediately', weight: 0.95 },
    ...
    sexual: [
        ...
        { text: 'Record victim statement under CrPC 164 before magistrate', weight: 0.9 },
    ...
    extortion: [
        ...
        { text: 'Check suspect criminal history for similar MO', weight: 0.75 }
    ],
    publicorder: [
        { text: 'Collect and preserve all video footage from scene', weight: 0.9 },
        { text: 'Identify and arrest named accused', weight: 0.85 },
...
function generateCases() {
    ...
    for (let i = 1001; i <= 1020; i++) {
        ...
        const timerStart = Math.max(0, 72 - Math.floor(hoursSinceFiled / 24));
        ...
        const leads = LEAD_TEMPLATES[crimeType].map((t, li) => ({
            id: `lead-${i}-${li}`,
            text: t.text,
            relevance: t.weight + (Math.random() * 0.1 - 0.05),
...
app.get('/case-management/cases', async (req, res) => {
    try {
        ...
        res.status(200).json({ cases: filtered, total: filtered.length });
    } catch (err) {
        console.error(err);
...
app.get('/case-management/cases/expiring', async (req, res) => {
    try {
        ...
        res.status(200).json({ cases: expiring, total: expiring.length });
    } catch (err) {
        console.error(err);
...
app.put('/case-management/cases/:id/stage', async (req, res) => {
    try {
        ...
        c.stage = stage;
        res.status(200).json({ caseId: c.caseId, stage: c.stage });
    } catch (err) {
        console.error(err);
...
app.put('/case-management/cases/:id/checklist/:itemId', async (req, res) => {
    try {
        ...
        res.status(200).json({ caseId: c.caseId, checklistCompletion: c.checklistCompletion, checklist: c.checklist });
    } catch (err) {
        console.error(err);
...
app.get('/case-management/cases/:id/leads', async (req, res) => {
    try {
        ...
        res.status(200).json({ leads: sorted });
    } catch (err) {
        console.error(err);
...
app.get('/case-management/stats', async (req, res) => {
    try {
        ...
        res.status(200).json({ stageCounts: stats, expiringCases: expiringCount, avgChecklistCompletion: avgCompletion });
    } catch (err) {
        console.error(err);
...
app.get('/case-management/notifications', async (req, res) => {
    try {
        ...
        res.status(200).json({ notifications: all, total: all.length });
    } catch (err) {
        console.error(err);
